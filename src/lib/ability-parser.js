 ...
 * 評価値はここでは出さない。基準値と補正は src/data/ability-rubric.json（別PR）。
 *
...
const ATOMS = [
  ...
  ['ガッツダメ上昇', /ガッツダメ(?:ージ)?上昇Lv(\d+)|ガッツダメ(?:ージ)?<?\+?(\d+)%?>?(?:上昇)?/],
  ['与ダメ上昇', /(?:与ダメ(?:ージ)?|技ダメージ|特殊与ダメ|(?<!ガッツ|被|クリ|クリティカル)ダメ)上昇Lv(\d+)|(?:与ダメ(?:ージ)?|技ダメージ)(?:を)?<?\+?(\d+)%>?(?:上昇|増加)|ダメージ<?\+(\d+)%>?|ダメージ(?:を|に)?(\d+)%(?:上昇|乗算)|与ダメージ(?:が)?上昇(?:<最大(\d+)%>)?|ダメージ上昇<最大(\d+)%>/],
  ...
  ['追撃', /(?:ライフ|最大ライフ|ちから|かしこさ|命中|回避|丈夫さ|与ダメ(?:ージ)?|ダメージ|相手のちから|技ダメージ)?(?:ステ(?:ータス)?)?(?:の|から)?<?(\d+)%>?(?:分)?(?:の|で)(?:<\d+回>)?追撃(?:を<\d+回>(?:与える|追加)|<\d+回>|を与える|を追加|ダメージ)?|(?:の|で)?<(\d+)回>追撃|追撃<(\d+)回>|追撃を<(\d+)回>(?:与える|追加)|追撃Lv(\d+)|(?:で|の)追撃(?:を与える|ダメージ|を追加)?|追撃ダメージ(?:Lv)?(\d+)?|(?:最大ライフの)?(\d+)%の追加ダメージ(?:を与える)?/],
  ...
  ['ステ変換', /(?:ライフ|ちから|かしこさ|命中|回避|丈夫さ)(?:の)?<?(\d+)%>?を(?:かしこさ|ちから|命中|回避|丈夫さ|ライフ)(?:ステータス)?に加算|(?:人気度|忠誠度)\d+を超えた分を(?:命中率|攻撃ステータス)に加算/],
  ['命中率上昇', /命中(?:率)?上昇Lv(\d+)|命中率(?:を|に)?<?\+?(\d+)%>?(?:上昇|加算)?|命中率(?:が)?上昇/],
  ['回避率上昇', /回避(?:率)?上昇Lv(\d+)|回避率<?\+?(\d+)%>?(?:上昇)?/],
  ...
  ['シールド破壊', /【?シールド】?(?:を)?破壊/],
  ['ガッツ回復', /ガッツ回復Lv(\d+)|ガッツ回復(?:速度)?(?:を)?<?\+?(\d+)%>?(?:上昇)?|ガッツ速度上昇Lv(\d+)|ガッツ<\+(\d+)>|ガッツ(?:を)?(\d+)回復|ガッツ回復(?!を停止)/],
  ['相手ガッツ停止', /相手のガッツ回復を停止|ガッツ回復を停止/],
  ['相手ガッツ減少', /相手のガッツ(?:を)?<?-?(\d+)>?(?:減少|低下|にする)|所持オーラ種×<(\d+)>ガッツダメ/],
  ['ガッツダメ低下', /被ガッツダメ(?:ージ)?(?:減少|低下)Lv(\d+)|被ガッツダメ(?:ージ)?(?:を)?<?-?(\d+)%?>?(?:減少|低下)?/],
  ['消費ガッツ', /消費ガッツ上昇Lv(\d+)|消費ガッツ(?:を|量)?<?([-+]?\d+)%?>?(?:減少|上昇|低下)?|ガッツ消費量(\d+)%(?:上昇|減少)/],
  ['相手忠誠度減少', /相手の忠誠度を(\d+)%減少/],
  ['相手被ダメ上昇', /相手(?:の|に)被ダメ(?:ージ)?上昇Lv(\d+)|相手(?:の|に)被ダメ(?:ージ)?<?\+?(\d+)%>?|被ダメージ上昇<最大(\d+)%>/],
  ['相手ステ低下', /相手の?(?:攻撃ステータス|ライフ以外のステ(?:ータス)?|全ステ(?:ータス)?|ちから(?:と|・)?かしこさ(?:ステ)?|回避(?:と|・)?命中(?:ステ)?|ちから|かしこさ|命中|回避|丈夫さ|命中率|回避率)(?:ステ(?:ータス)?)?(?:を)?<?-?(\d+)%>?(?:減少|低下|吸収)?|(?:ライフ以外のステ|ちからとかしこさステ|回避と命中ステ)<-(\d+)%>|相手の攻撃ステータス減少<-(\d+)%>|相手の攻撃ステータスをちからとかしこさの低い方にする/],
  ['封じ', /使用不可|使用(?:でき|出来)なく|追撃封鎖|封じ/],
  ['無効化・解除', /(?:追撃|連撃|状態変化|デバフ|能力|効果)(?:\/(?:追撃|連撃))?(?:を)?(?:無効|解除)/],
  ['食いしばり', /(?:通常\/追撃\/連撃)?ダメージを受けた時ライフ1で耐える|ライフ1で耐える/],
  ['オーラ変貌', /有利オーラに変貌|有利オーラ(?:に)?変換|オーラ有利(?:に|として)/],
  ['適性効果上昇', /(?:近|中|遠|零)?距離?適性効果上昇Lv(\d+)|地形適性効果上昇Lv(\d+)|適性効果(?:を)?<?\+?(\d+)%>?/],
  ['ライフ回復', /ライフ(?:を)?<?(\d+)%?>?回復|ライフ(?:が)?回復/],
  ['貫通', /貫通Lv(\d+)|貫通/],
  ['技ステ上昇', /技ステータス<?\+(\d+)>?|すべての技ステータス上昇Lv(\d+)|技ステ(?:ータス)?(?:が)?上昇/],
  ['ステ上昇', /(?:ライフを除く|ライフ以外の)?(?:全ステ(?:ータス)?|ライフ以外のステ(?:ータス)?)(?:を)?<?\+?(\d+)%>?(?:上昇)?|(?:攻撃ステータス|ちから|かしこさ|命中|回避|丈夫さ|ライフ)(?:ステ(?:ータス)?)?(?:を|が)?<(\d+)%>上昇|(?:攻撃ステータス|ちから|かしこさ|命中|回避|丈夫さ|ライフ)(?:ステ(?:ータス)?)?上昇<\+(\d+)%>|(?:攻撃ステータス|ちから|かしこさ|命中|回避|丈夫さ|ライフ)(?:ステ(?:ータス)?)?(?:を|が)?<?\+(\d+)%?>?(?:上昇)?|(?:攻撃ステータス|ちから|かしこさ|命中|回避|丈夫さ|ライフ)(?:ステ(?:ータス)?)?(?:を|が)?(\d+)%上昇|(?:ちから|かしこさ|丈夫さ|命中|回避|ライフ)UP\s?Lv(\d+)|基礎(?:丈夫さ|ちから|かしこさ|命中|回避|ライフ)ステ\+(\d+)|(?:攻撃ステータス|ちから|かしこさ|命中|回避|丈夫さ|ライフ)(?:ステ(?:ータス)?)?(?:が)?上昇(?:Lv(\d+)|<最大(\d+)%>)?/],
  ['クリ率上昇', /クリ率大上昇/],
  ['命中率上昇', /命中率大上昇/],
  ['回避率上昇', /回避率大上昇/],
...
const TRIGGERS = [
  ['技非命中時', /技が命中しなかった(?:とき|時)|命中しなかった(?:とき|時)/],
  ['技命中時', /技命中時|技が命中した(?:とき|時)|命中時|命中したとき/],
  ['技発動時', /技発動時|技を発動した(?:とき|時)|技発動で|技発動中/],
  ['被ダメ時', /被ダメ(?:した)?(?:とき|時)|技を受けた時|ダメージを受けた時|ガッツダメージ受けた時/],
  ['回避時', /回避した(?:とき|時)|技を回避したとき/],
  ['クリティカル時', /クリティカル(?:発生)?時/],
  ['バトル開始時', /バトル開始時/],
...
function parseLine(rawLine) {
  ...
  text = text.replace(/\[([^\]]+)\]/g, (m, inner) => {
    ...
    if (c.kind === 'apply') { line.apply = c.value; return ' '; }
  ...
  const apt = text.match(/(?:間合い)?(?:適性)?(SS|S|A)(以上|未満)(?:：|の間合いで|技)?/); if (apt) { line.skillCond.aptitude = apt[1] + apt[2]; text = text.replace(apt[0], ' '); }
  ...
  if (/つ前と同じ技|同じ技を使用|連続で使用|1回前が/.test(text)) { line.skillCond.repeat = true; text = text.replace(/(?:\d)?つ前と同じ技を使用した(?:とき|時)|同じオーラ技を連続で使用した(?:とき|時)|1回前が[^、]*(?:時|とき)/, ' '); }
  // 3. トリガー
  for (const [name, re] of TRIGGERS) { if (re.test(text)) { line.trigger = name; text = text.replace(re, ' '); break; } }
  // 4. 散文の状況条件
  const prose = [
    ['compare', /相手より(?:も)?(?:[^、\s]+?)(?:ステータス)?が(?:高い|多い)(?:とき|時|と)?/],
    ['life', /(?:自身の)?(?:残り)?ライフ(?:が)?\d+%(?:以上|以下|未満|の時)(?:かつ|、|で|の時|時)?|残りライフが(?:少ない|多い)(?:ほど|とき|時)|相手ライフが少ない時|ライフが相手より高いと|残りライフが相手より多い時/],
    ['stance', /(?:優勢|劣勢)のとき|優勢時|劣勢時/],
    ['weather', /天候が(?:晴れ|曇り|悪天候)のとき/],
    ['loyalty', /(?:忠誠度|人気度)<?\d+>?(?:以上)?(?:かつ)?[^、]*?(?:超えた分を)?/],
    ['stack', /スタック(?:が)?<?\d+>?(?:になり)?|地形適性に応じて|技のRに応じて|回数に応じて|に応じて|(?:\d+)個につき/],
    ['probability', /<(\d+)%>の確率で|稀に/],
    ['while', /(?:この能力の)?【?シールド】?破壊まで|発動中|この効果中/],
  ...
  for (const [atom, re] of ATOMS) {
    ...
    rest = rest.replace(new RegExp(re.source, 'g'), (...m) => {
      ...
      const stats = /全ステ|ライフ以外のステ|ライフを除く/.test(m[0])
