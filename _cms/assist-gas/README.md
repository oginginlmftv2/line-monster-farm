...
`sourceOrder`は元JSONの配列順、`version`は同時編集検出に使う。カードの`cardId`、能力の
...
- カード: 基本属性、評価、ステータス、解説、編成をカード単位で保存
- 効果: 1カードの効果配列を全置換。空配列は`draft`として残す
- 能力: abilityId検索またはカード詳細から1件ずつ保存
