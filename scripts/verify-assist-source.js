...
function verifyGenerated() {
  ...
  const unexpected = paths.filter(path => !GENERATED_PREFIXES.some(prefix => path.startsWith(prefix))
...
if (mode === 'source' && first && second) {
  ...
  console.error('使い方:');
  console.error('  node scripts/verify-assist-source.js source <source-ref> <base-ref>');
  console.error('  node scripts/verify-assist-source.js generated');
  process.exitCode = 2;
}
