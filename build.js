...
const NON_BUFF_TOKENS = [
  ...
  '相手魔族以外', '自身モン類有利', '自身オーラ有利',
  // 技能力テキスト内の状態表記だが、ゲーム内ヘルプのバフ・デバフ一覧には無い
  'プラズマ', '楔', '武天', '武天（連撃与ダメ）', '王者覚醒',
  // オーラ・モン類の相性を指す発動条件（[有利]と同類）。バフではない
  'オーラ有利', 'モン類有利',
  // ゴースト種RSFの手札状態（[RSF：J（…）] など）。ヘルプのバフ・デバフ一覧には無い
  'RSF：J（すべての技命中時、ダメ追撃）', 'RSF：Q（追撃与ダメ）', 'RSF：K（ガッツダメ）', 'RSF：A（攻撃ステ）',
  // 間合い適性を指す発動条件（例 [遠距離]適性に応じて）。バフではない
  '遠距離', '中距離', '近距離', '零距離',
  // 技能力テキスト内の状態表記だが、ヘルプのバフ一覧には無い（例 [ブロックLv6＜20秒＞]）
  'ブロック',
  // 相手のサイズを指す発動条件（例 相手サイズ[大きい]）。バフではない
  '大きい', '小さい',
];
