...
| P15-4b | 能力スコアリング 第1段：説明文パーサとカバレッジ表 | `feat/p15-4b-ability-parser` | **レビュー待ち** | ⚪ | 設計は`docs/ability-scoring-design.md`が正。適用条件（hard）／技条件（soft）／状況条件／トリガー／効果原子（大分類6）／ヒット数と回数制限を構造化。評価対象829件（I→II・III→IVは上位のみ）のうち全行抽出603・一部208・なし18。評価値・Tier・ページ描画は第2段。公開物は無変更 |
