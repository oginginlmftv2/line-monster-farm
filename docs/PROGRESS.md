...
- **generatedFrom の期待値を単値へ締める（PR-C。段階7の後）:**
  このPRで `scripts/verify.js` は `['P12-8 test assist CMS']` と `['ライ徹CMS']` の
  2値を許す。段階7でアシスト公開が成功し3DBが `['ライ徹CMS']` になったら、
  別PRで `['ライ徹CMS']` の単値へ締める。
  締めないと、古い由来表記へ戻る退行を検査が見逃す。
