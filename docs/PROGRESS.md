...
| P12-11 | CMS統合の実装（段階1〜5d） | `chore/p12-11-s1-cms-token-scan` / `chore/p12-11-s2-cms-unified-source` / `fix/p12-11-s2b-assist-publish-scope` / `chore/p12-11-s3-assist-db-from-cms` / `feat/p12-11-s4-assist-publish-path` / `docs/p12-11-s4b-route-verified` / `fix/p12-11-s5b-shell-defects` / `fix/p12-11-s5c-shell-layout` / `fix/p12-11-s5d-user-feedback` | **進行中** | ⚪🔴🟡 | 段階1〜4B・5b・5cは完了。段階5のリハーサル項目7・8で利用者への通知とOCR部分成功の取りこぼしを検出して中断し、段階5dで是正中。是正後のソースを貼り直し、段階5を続行する |
...
現在は**段階5実施中・段階5d（利用者への通知とOCR部分成功の取りこぼし是正）進行中**である。

- 段階4Bでは正常経路が全ステップ成功し、拒否3ケースも最初の門で期待どおり停止した
- 正常経路によりmainは`4e1de69`へfast-forwardしたが、treeは段階4A直後の`5c6f4a5`と同一
- 段階5cでレイアウト不具合を是正後、リハーサル項目7・8で拒否理由が画面外に表示され、OCRの成功済み3枚分も後続1枚の失敗で失われることを検出して再度中断した
- 段階5dは管理者の承認取得済み。段階6以降は管理者の明示承認なしに着手しない
...
- 段階5: **実施中・中断** — 本番bookのコピーでのリハーサル項目7・8により利用者への通知とOCR部分成功の取りこぼしを検出。段階5dの是正後に続行する（⚪）
...
- 段階5d: **進行中** — シェル共通通知とOCRのファイル単位の成否処理を追加し、利用者へ失敗理由と部分成功を必ず届ける（⚪）`fix/p12-11-s5d-user-feedback`
