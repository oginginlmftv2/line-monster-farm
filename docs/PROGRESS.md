...
| P15-1 | GSC調査とソフト404・404の解消 | `fix/p15-1-soft404-cleanup` | **完了** | 🔴 | PR #141をmainへマージ（`989a272`）。404.htmlを新設し、cards/card.htmlのソフト404とmonsters/monster.htmlの広告掲載を是正。検査14を旧URL共通ページ2枚共通へ拡張 |
| P15-2 | 本文の乏しいページの広告タグ撤去 | `fix/p15-2-thin-page-ads` | **完了** | 🔴 | PR #143をmainへマージ（`7032374`）。8ページから広告タグを削除（7枚は実際に読み込まれていた）。検査20を新設しサイト全体で機械検査 |
| P15-3 | AdSense申請・インデックス改善の引き継ぎ文書 | `chore/p15-3-adsense-handoff` | **レビュー待ち** | ⚪ | `docs/adsense-indexing-handoff.md`を新設。GSC分類の結論、誤報の記録、能力スコアリング方針の合意事項を残した。文書のみで公開物は無変更 |
| P15-4a | モンスター基礎データ（素質・特徴・適性）DBと詳細ページの基礎データセクション | `feat/p15-4-monster-basics` | **レビュー待ち** | 🔴 | 2026-09-17に4回目のAdSense不合格（理由は前回と同じ「有用性の低いコンテンツ」）。主血統×オーラでは最大6体が同一内容になるため、スクショから読む基礎データで1体ごとに固有化。`src/lib/monster-basics.js`に評価式を隔離、TSV取り込み・スキル・検査21を新設。初期データ6体。基礎データだけでは+310字止まりで800字には届かない（設計書3章） |
| P15-4b | 能力スコアリング 第1段：説明文パーサとカバレッジ表 | `feat/p15-4b-ability-parser` | **レビュー待ち** | ⚪ | 設計は`docs/ability-scoring-design.md`が正。適用条件（hard）／技条件（soft）／状況条件／トリガー／効果原子（大分類6）／ヒット数と回数制限を構造化。評価対象829件（I→II・III→IVは上位のみ）のうち全行抽出603・一部208・なし18。ヒット数46行の読みは2026-09-26に確定、抽出なし18件はゲーム側の効果詳細を見てから再検討のため保留。評価値・Tier・ページ描画は第2段。公開物は無変更 |
...
今回の公開表示差分はマーカーコメントとリセマラの固定カード名を除く汎用化だけである。

G1として、ガチャDBと静的ページ生成の基盤を実装した。本番の`gachas.json`は空で、
既存HTMLとsitemapの出力を変えず、fixtureだけで詳細・一覧・二軸ゲート・決定性を検証する。
設計と可視文字数の実測は`docs/gacha-design.md`を正とする。
...
| 日付 | タスクID | 内容 | 対応 |
|---|---|---|---|
| 2026-08-27 | P12-16 | setup3_importAssistFromMain が abilities シートで「固定されていない行をすべて削除することはできません」で停止した。1行目が固定され、最終データ行が最終物理行と一致していると、deleteRows(2, getLastRow()-1) が固定行以外の全行削除になり拒否される。cards 91行・effects 896行は物理行に余裕があり通過していた | deleteRows をやめ、insertRowsAfter で行数を確保し clearContent で消してから setValues する形へ変えた。旧実装へ戻す破壊テストを追加した |
