...
| G6 | ガチャ公開日時の形式不一致修正 | `fix/g6-gacha-datetime-format` | **レビュー待ち** | 🟡🔴 | GASの秒付きJST出力とbuildの分単位契約が不一致で初回公開が停止。既存秒固定値を分単位へ移行し、再発検査を追加 |
...
G6として、初回ガチャ公開で判明したGASの秒付き日時と`build.js`の分単位日時の契約不一致を
修正する。既存シート値はGAS読取時に分単位へ移行し、build側の正規形式は変更しない。
