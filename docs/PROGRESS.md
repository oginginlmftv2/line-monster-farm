...
| P12-11 | CMS統合の実装（段階1〜5c） | `chore/p12-11-s1-cms-token-scan` / `chore/p12-11-s2-cms-unified-source` / `fix/p12-11-s2b-assist-publish-scope` / `chore/p12-11-s3-assist-db-from-cms` / `feat/p12-11-s4-assist-publish-path` / `docs/p12-11-s4b-route-verified` / `fix/p12-11-s5b-shell-defects` / `fix/p12-11-s5c-shell-layout` | **進行中** | ⚪🔴🟡 | 段階1〜4B・5bは完了。段階5の再リハーサルでシェルのレイアウト崩れを検出して中断し、段階5cで是正中。是正後のソースを貼り直し、段階5を続行する |
...
現在は**段階5実施中・段階5c（再リハーサルで検出したレイアウト不具合の是正）進行中**である。

- 段階4Bでは正常経路が全ステップ成功し、拒否3ケースも最初の門で期待どおり停止した
- 正常経路によりmainは`4e1de69`へfast-forwardしたが、treeは段階4A直後の`5c6f4a5`と同一
- 段階5bで3件を是正後、再リハーサルでモンスタータブのレイアウト崩れを検出して再度中断した
- 段階5cは管理者の承認取得済み。段階6以降は管理者の明示承認なしに着手しない
...
- 段階5: **実施中・中断** — 本番bookのコピーでの再リハーサルでモンスタータブのレイアウト崩れを検出。段階5cの是正後に続行する（⚪）
- 段階5b: **完了** — タブとパネルのid不一致、setup結果のログ欠落、header配色競合を是正し、再発検査を追加（⚪）PR #52 / `8471e4c`
- 段階5c: **進行中** — アシスト用の裸のタグセレクタがシェルへ波及するレイアウト不具合を是正し、再発検査を追加する（⚪）`fix/p12-11-s5c-shell-layout`
