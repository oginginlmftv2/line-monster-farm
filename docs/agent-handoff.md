...
4  括弧を全角へ統一
5  読点後の半角スペース削除、全角括弧前の半角スペース削除
6  英字の II / III を Ⅱ / Ⅲ へ
7  + の前後  見出し（name）は + の前に半角スペース1つ
             説明文（description）は前後ともスペースなし
             description は行ごとにサニタイズし改行を保つ
```

4〜7は**取り込み時のサニタイザだけ**の責務で、`scripts/verify.js`では検査しない。
文字形の揺れはデータの誤りではなく、OCRの誤りパターンを事前に網羅できないため、
検証で落とすとCMS公開が予測不能に失敗する。verifyが落とすのは
`MAX↑`と行頭`•`＝**OCRのUI片が本文へ混入した場合だけ**に限定してある。

ただし**1〜7を片方の実装だけ変更するとFAIL**する。`scripts/verify-assist-cms.js`の
`ocrNormalizationRules` / `ocrFieldSanitizeRules`が両実装を突き合わせている。
ルールを足すときは実装2箇所＋この突き合わせ表＋`scripts/test-verify-assist-cms.js`を同時に更新する。
