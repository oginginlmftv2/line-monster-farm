...
verifierが固定検査する。`status`（まとめ更新で変わる）と`cardId / sortOrder / linkStatus`
（`api_asstUnlinkAbility`の紐付け解除・並び替え・再紐付けで変わる）は運用で変わる値なので固定対象外。監査時点では新規由来の`legacyId: null`と`status: draft`は0件であり、
