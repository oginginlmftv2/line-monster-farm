...
3. 本番bookのコピーを使う場合、`ENVIRONMENT=rehearsal` と `members!A1` の環境マーカーが
   リハーサル側を指している
4. 「＋ 新規カード」から必須属性を入力すると、確認ダイアログにcardIdが自動採番される旨と
   カード名・レアリティ・未公開の案内が出る
5. 登録後に一覧件数が1件増え、作成カードの既存編集画面が開き、
   採番されたcardIdをファイル名にする画像追加の案内が出る
...
9. 採番されたcardIdが`c<4桁連番>-<レアリティ>`で、既存の最大連番+1である
10. 同じ「カード名 + レアリティ」が拒否される

確認用の行を本番へ増やさない。実運用で登録する1件をそのまま確認に使う。

## 3. cardId の規則

新規カードのcardIdは**サーバー側の自動採番**で、管理者は入力しません。

```
新形式（自動採番）  c<4桁連番>-<MR|SSR>        例 c0092-MR
旧形式（既存91件）  ^[a-z][a-z0-9]*-(MR|SSR)-[a-z0-9]+$
```

- 連番は`cards`シートの既存cardIdのうち新形式のものの最大値+1。sourceOrderからは採番しない
  （行削除でsourceOrderが再利用されると、消えたカードの旧URLと衝突しうるため）
- 採番はScript Lock取得後の再読込で確定するため、並行登録でも重複しない
- 旧形式の接頭辞に意味はなく、コード上もパースしていない。既存IDは公開URL
  （`cards/<cardId>.html`）・画像ファイル名・Firestoreキーの実体なので**変更しない**
- レアリティ部分はcard.rarityと一致する。`scripts/verify.js`が新旧書式と一致を全件検査する
