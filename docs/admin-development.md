...
## 10. 次回の実更新で一連の手順を確認する

文書1行だけの人工的な通し稽古は保留する。Git、Node.js、GitHubのPR・マージ経路は
P10-1で動作を確認できたため、第2段階の実際の通常ページ更新で同じ手順を確認する。

次回は`docs/claude-next-session.md`をClaudeへ渡し、更新内容に応じたタスクIDとブランチ名が
提示されてから作業する。問題が起きた場合だけ、第1段階を再開して本書へ反映する。
